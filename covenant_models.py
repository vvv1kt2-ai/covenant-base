...
def categorize_covenant(text):
    ...
    # Кросс-дефолт — ДО Долговой нагрузки: формула кросс-дефолта содержит
    # «иным долговым обязательствам», и общее слово «долгов» из долговой
    # нагрузки перетягивало бы эти кейсы в неверную категорию
    # (специфичные категории проверяются раньше общих, как «Делистинг» выше)
    if any(kw in lower for kw in [
